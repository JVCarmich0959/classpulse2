...
// ── SUPABASE CONFIG ──
var SB_URL = 'https://kcveniekiinppbbytjrs.supabase.co';
var SB_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtjdmVuaWVraWlucHBiYnl0anJzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU4ODA2NzUsImV4cCI6MjA5MTQ1NjY3NX0.KFAS6bVzAPjnkA_bd9j3zARcxwp2-EBypN9FquViYmg';
...
var SESSION = {token: null, email: null, userId: null, role: null};
function saveSession(token, email, userId){
  SESSION.token = token; SESSION.email = email; SESSION.userId = userId || null;
  try{sessionStorage.setItem('sb_token', token); sessionStorage.setItem('sb_email', email); if(userId) sessionStorage.setItem('sb_uid', userId);}catch(e){}
}
function loadSession(){
  try{var t=sessionStorage.getItem('sb_token');var e=sessionStorage.getItem('sb_email');var u=sessionStorage.getItem('sb_uid');if(t){SESSION.token=t;SESSION.email=e;SESSION.userId=u||null;return true;}}catch(e){}
  return false;
}
...
function pb(pct,col){return '<div class="pbar"><div class="pfill" style="width:'+Math.min(pct,100)+'%;background:'+col+'"></div></div>';}
...
function initFreshness(){
  ...
  fetchLiveData(function(err, rows){
    ...
    if(!err && rows){
      ...
      if(f) f.textContent='Supabase unreachable · cached data';
      var fd=el('fresh-dot');if(fd) fd.style.background='var(--amber)';
    }
  });
}
  ...
  btnLogin.addEventListener('click', function(){
    ...
    sbSignIn(email, pass).then(function(r){ return r.json(); }).then(function(data){
      if(data.access_token){
        saveSession(data.access_token, email, data.user && data.user.id);
...
// ── SIGN OUT ──
function signOut(){
  SESSION.token = null; SESSION.email = null; SESSION.userId = null; SESSION.role = null;
  try{sessionStorage.removeItem('sb_token');sessionStorage.removeItem('sb_email');sessionStorage.removeItem('sb_uid');}catch(e){}
...
function renderAdmin(){
  ...
  var bbtn=el('btn-backfill');if(bbtn)bbtn.addEventListener('click',runBehaviorBackfill);
}
...
// ── BEHAVIOR BACKFILL ──
var BACKFILL_RUNNING=false;
function runBehaviorBackfill(){
  if(BACKFILL_RUNNING)return;
  BACKFILL_RUNNING=true;
  var statusEl=el('backfill-status');
  if(statusEl) statusEl.textContent='Fetching untagged rows…';
  var tok=SESSION.token||SB_KEY;
  // fetch rows with empty behaviors array
  fetch(SB_URL+'/rest/v1/incidents?behaviors=eq.%7B%7D&select=id,notes,student&limit=50',{
    headers:{'apikey':SB_KEY,'Authorization':'Bearer '+tok}
  }).then(function(r){return r.json();})
  .then(function(rows){
    if(!rows||!rows.length){
      if(statusEl) statusEl.textContent='All rows already tagged.';
      BACKFILL_RUNNING=false;
      return;
    }
    if(statusEl) statusEl.textContent='Tagging '+rows.length+' rows…';
    var done=0,failed=0;
    var BEH_LIST=BEHAVIORS.join(', ');
    function processNext(i){
      if(i>=rows.length){
        if(statusEl) statusEl.textContent='Done — tagged '+(done)+', failed '+failed+'.';
        BACKFILL_RUNNING=false;
        STATE.liveLoaded=false;STATE.liveRows=[];
        return;
      }
      var row=rows[i];
      var prompt='Given this incident note, return ONLY a JSON array of applicable behavior tags from this list: ['+BEH_LIST+']. Return [] if none clearly apply. Note: "'+((row.notes||'').slice(0,300)).replace(/"/g,"'")+'"';
      ...
      .then(function(data){
        ...
        var tags=[];
        if(match){try{tags=JSON.parse(match[0]).filter(function(t){return BEHAVIORS.indexOf(t)>=0;});}catch(e){}}
        return fetch(SB_URL+'/rest/v1/incidents?id=eq.'+row.id,{
          method:'PATCH',
          headers:{'apikey':SB_KEY,'Authorization':'Bearer '+tok,'Content-Type':'application/json','Prefer':'return=minimal'},
          body:JSON.stringify({behaviors:tags})
        });
      }).then(function(){
        done++;
        if(statusEl) statusEl.textContent='Tagged '+done+'/'+rows.length+'…';
    ...
    if(statusEl) statusEl.textContent='Error fetching rows.';
    BACKFILL_RUNNING=false;
  });
}
...
el('btn-det-back').addEventListener('click',function(){showScreen('S-classes',true);});
...
el('es-save').addEventListener('click',function(){
  ...
  if(EDIT_STATE.dbId){
    ...
    fetch(SB_URL+'/rest/v1/incidents?id=eq.'+EDIT_STATE.dbId,{
      ...
      if(!r.ok) throw new Error('HTTP '+r.status);
      // update local cache
      STATE.myDbLogs.forEach(function(row,i){
        if(String(row.id)===String(EDIT_STATE.dbId)){
          Object.assign(STATE.myDbLogs[i],{student:updates.student,homeroom:updates.homeroom,specials:updates.specials,behaviors:updates.behaviors,incident_date:updates.incident_date,incident_time:updates.incident_time,color_chart:updates.color_chart,home_contact:updates.home_contact,notes:updates.notes});
        }
      });
      closeEditSheet();
      STATE.liveLoaded=false;STATE.liveRows=[];
      if(EDIT_STATE.onAfterEdit) EDIT_STATE.onAfterEdit();
      else renderHistory();
    }).catch(function(err){
      status.textContent='Save failed — '+err.message;status.style.color='var(--red)';
      saveBtn.textContent='[ Save changes ]';saveBtn.disabled=false;
    });
  } else {
    // session-only log — update in STATE.logs
    STATE.logs.forEach(function(l){
      if(('s-'+l.id)===EDIT_STATE.uid){
        l.studentName=updates.student;l.homeroom=updates.homeroom;l.specials=updates.specials;
        l.behaviors=updates.behaviors;l.date=updates.incident_date;l.time=updates.incident_time;
        l.colorChart=updates.color_chart;l.homeContact=updates.home_contact;l.notes=updates.notes;
      }
    });
    closeEditSheet();
    if(EDIT_STATE.onAfterEdit) EDIT_STATE.onAfterEdit();
...
el('del-go-btn').addEventListener('click',function(){
  ...
  fetch(SB_URL+'/rest/v1/incidents?id=eq.'+DEL_STATE.dbId,{
    method:'DELETE',
    headers:{'apikey':SB_KEY,'Authorization':'Bearer '+tok,'Prefer':'return=minimal'}
  }).then(function(r){
    if(!r.ok) throw new Error('HTTP '+r.status);
